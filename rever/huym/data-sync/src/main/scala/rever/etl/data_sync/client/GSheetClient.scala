...
case class GSheetClient(credentials: GoogleCredentials) {
  ...
  private def normalizeRefRecords(vr: ValueRange): Seq[Seq[AnyRef]]= {
    val v = vr.getValues
    if (v != null) {
      v.asScala
        .map(_.asScala.toSeq)
  ...
  private def normalizeRecords(vr: ValueRange): Seq[Seq[Any]]= {
    val v = vr.getValues
    if (v != null) {
      v.asScala
        .map(_.asScala.toSeq)
  ...
  private def isEmptyRow(row: Seq[Any]): Boolean = {
    !row
      .filterNot(_ == null)
      .exists(x => x.toString.trim.nonEmpty && !x.equals("-"))
  }
