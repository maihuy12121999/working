...
/** @author anhlt (andy)
  */
object PropertyESToClickhouseRunner {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val config = Config
      ...
      .addArgument("target_property_table", true)
      .addArgument("merge_after_write", false)
      .addArgument("DATA_HEALTH_LISTING_SCORE_CONFIG", false)
      .addArgument("is_sync_all", false)
      .build(Utils.parseArgToMap(args).asJava, true)

    rever_search.PropertyToCH(config).run()
