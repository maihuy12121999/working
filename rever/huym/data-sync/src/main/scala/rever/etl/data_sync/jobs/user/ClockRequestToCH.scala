...
import rever.etl.data_sync.domain.user_activity.ClockInOutRequest
import rever.etl.rsparkflow.api.configuration.Config

/** @author anhlt (andy)
  * @since 06/07/2022
  */
case class ClockRequestToCH(config: Config) extends Runner {
  ...
  override def run(): Long = {
    ...
    val source = MySqlSource(
      config,
      sourceTable,
      ClockInOutRequest.PRIMARY_IDS,
      ClockInOutRequest.FIELDS,
      ClockInOutRequest.TIME,
      fromTime,
      toTime,
      sourceBatchSize
    )

    val normalizer = ClockRequestNormalizer()
