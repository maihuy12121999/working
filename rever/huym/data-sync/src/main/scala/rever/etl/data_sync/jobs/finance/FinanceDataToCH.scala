...
case class DataRangeConfig(
    ...
    buActualRange: Option[String] = Some("C2:CA33"),
    costCompositionRange: Option[String] = Some("C43:CA78"),
    buBudgetRange: Option[String] = Some("C1:Z48"),
    dealRange: Option[String] = Some("G2:BK16"),
...
case class FinanceDataToCH(config: Config) extends Runner {
  ...
  override def run(): Long = {
    ...
    if (totalCount > 0) {
      brokerageFinanceDAO.optimizeTable(true)
      nonBrokerageFinanceDAO.optimizeTable(true)
      platformFinanceDAO.optimizeTable(true)
      buBrokerageDAO.optimizeTable(true)
      dealDAO.optimizeTable(true)
      costDAO.optimizeTable(true)
      bookingDAO.optimizeTable(true)
      byLevelDAO.optimizeTable(true)
      boTeamSizeDAO.optimizeTable(true)
      boCostDAO.optimizeTable(true)
      incomeByLevelDAO.optimizeTable(true)
    }
