package rever.etl.agentcrm.page_view.reader
...
class PageViewReader extends SourceReader {
  ...
  private def initQuery(config: Config): String = {
    ...
    s"""
       ...
       |    AND (url like '%agentcrm-stag.rever.vn%' 
       |          OR url like '%agentcrm-beta.rever.vn%' 
       |          OR url like '%agentcrm.rever.vn%'
       |         )
       |""".stripMargin
  }
}
